(function (global) {
  ...
  function normalizeBackWallGrid(rawGrid, legacySegments, depth, height) {
    ...
    const normalizedCells = array(source.cells).map((cell, index) => {
    ...
    return {
      ...
      cells: cells.length ? cells : [{ id: 'back_wall_cell_1', minX, maxX, minY, maxY }]
    };
  }
  ...
  function emptyScope(name, enabled) {
    return {
      ...
      supportPosts: [],
      parapetSegments: [],
      backWall: { xOffset: 0, depth: 600, height: 0, segments: [], grid: normalizeBackWallGrid(null, [], 600, 0) },
