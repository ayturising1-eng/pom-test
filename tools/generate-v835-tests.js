...
function buildGuillotine(rawPlacement) {
  const raw = {
    ...
    project: 'V8.3.5 GUILLOTINE TEST',
    systemCount: 1,
    width: '6800',
    opening: '4500',
    rearHeight: '3200',
...
if (!block.entities.some(e => Number(e.trueColor) === 2699657)) throw new Error('RGB 41,49,137 True Color uygulanmadı.');
...
if (!/420\s+2699657/.test(dxf.replace(/\r/g, ' '))) throw new Error('DXF True Color kaydı yok.');
const out = path.join(__dirname, '..', 'samples', 'v8_3_5-guillotine-test.dxf');
...
console.log(`OK v8.3.5 Guillotine: ${drawing.entities.length} entity, ${block.entities.length} block entity, ${Buffer.byteLength(dxf)} bytes`);
